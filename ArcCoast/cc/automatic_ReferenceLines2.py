...
def extract_shorelines(metadata, settings, stamp_shoreline):
    """
    ...
    for satname in metadata.keys():
        ...
        for i in range(len(filenames)):
            ...
            im_ref_buffer = create_shoreline_buffer(cloud_mask.shape, georef, image_epsg,
                                                    pixel_size, settings, stamp_shoreline)
            ...
            # map the contours automatically with one of the two following functions:
            ...
            if settings['check_detection'] or settings['save_figure']:
                date = filenames[i][:19]
                skip_image = show_detection(im_ms, cloud_mask, im_labels, shoreline, image_epsg, georef, settings, date, satname)
...
def create_shoreline_buffer(im_shape, georef, image_epsg, pixel_size, settings, stampfilename):
    """
    ...
    sf = shapefile.Reader(stampfilename)
    shaperecs = sf.shapeRecords()
    SL = np.array(shaperecs[0].shape.points)
    sf.close()
    ...
    if True:
        ...
        ref_sl = SL
        ref_sl_conv = SDS_tools.convert_epsg(ref_sl, SLepsg,image_epsg)
